...
def make_plot(result: dict, file_name: str) -> bool:
    ...
    plt.xticks(
        [r + bar_width for r in range(len(col_first))],
        result.keys(),
    )
...
def generate_partial_column_report(test_results: dict, file_name: str) -> bool:
    ...
    with open(f"files/{file_name}.md", "w") as f:
        ...
        for fun in result:
            f.write(f"## {fun}\n")
            f.write(
                f"**SCORE:** {round(result[fun][0] + result[fun][1] + result[fun][2], 3)}\n\n"
...
def similar_and_not_similar_file(
    file_name: str,
    test_results: dict,
    ...
    with open(f"files/{file_name}similar.md", "w") as f_sim:
        with open(
            f"files/{file_name}not_similar.md",
            "w",
        ) as f_not:
...
def generate_sim_report(test_results: dict, file_name: str):
    ...
    with open(
        f"files/{file_name}similar_and_not.md",
        "w",
    ) as f:
        for function in test_results.keys():
            score = 0
