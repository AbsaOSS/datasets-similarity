...
class TypeMetadataCreator(MetadataCreator):
    ...
    def from_settings(settings: AnalysisSettings) -> "MetadataCreator":
        ...
        if settings.correlation: metadata_creator.compute_correlation()
        if settings.incomplete_columns: metadata_creator.compute_incomplete_column()
        if settings.column_name_embeddings: metadata_creator.compute_column_names_embeddings()
        if settings.column_embeddings: metadata_creator.create_column_embeddings()
        if settings.kinds: metadata_creator.compute_column_kind()
        if settings.type_basic: metadata_creator.compute_basic_types()
        if settings.type_advanced: metadata_creator.compute_advanced_types()
        if settings.type_structural: metadata_creator.compute_advanced_structural_types()
