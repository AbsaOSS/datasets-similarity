...
from src.impl.comparator.utils import cosine_sim
from src.impl.metadata.type_metadata_creator import TypeMetadataCreator
from src.models.types_ import NONNUMERICAL
...
def get_nonnumerical_data(
    ...
    for i in files:
        index += 1
        data = pd.read_csv(i)
        metadata_creator = TypeMetadataCreator(data).compute_advanced_structural_types().compute_column_kind()
