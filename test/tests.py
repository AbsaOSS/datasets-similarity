...
# from functions import load__csv_files_from_folder, DataFrameMetadataCreator
#
#
...
#
#
#     #def test_similarity_of_metadata(self):
#         # database, names = load__csv_files_from_folder("../data")
#         # metadata: dict[str, DataFrameMetadata] = defaultdict()
#         # for dataframe, name in zip(database, names):
#         #     metadata[name] = DataFrameWithStat(dataframe).compute_correlation(
#         #         0.5).create_column_embeddings().get_metadata()
#         #     icompl = metadata[name].column_incomplete
#         #     categ = metadata[name].column_categorical
#         #
#         # metadata2: dict[str, DataFrameMetadata] = defaultdict()
#         # for dataframe, name in zip(database, names):
#         #     metadata2[name] = DataFrameWithStat(dataframe).compute_correlation(
#         #         0.5).create_column_embeddings().get_metadata()
#         #     icompl = metadata[name].column_incomplete
#         #     categ = metadata[name].column_categorical
#         # # self.assertEqual(metadata, metadata2)
#         # for m1, m2 in zip(metadata2.values(), metadata.values()):
#         #     self.assertEqual(m1, m2)
#         #
#         # database2, names2 = load__csv_files_from_folder("../data")
#         # metadata3: dict[str, DataFrameMetadata] = defaultdict()
#         # for dataframe, name in zip(database2, names2):
#         #     metadata3[name] = DataFrameWithStat(dataframe).compute_correlation(
#         #         0.5).create_column_embeddings().get_metadata()
#         #     icompl = metadata[name].column_incomplete
#         #     categ = metadata[name].column_categorical
#         #
#         # # self.assertEqual(metadata, metadata3)
#         # # self.assertEqual(metadata2, metadata3)
#         # for m1, m2 in zip(metadata3.values(), metadata.values()):
#         #     self.assertEqual(m1, m2)
#
#
#
